...
function ABcheck() {
  ...
  if (getCurrentAB(true) == 'pbs' &&
      (checkPreset(1) || checkPreset(2) || checkPreset(3))) {
...
function ABsolver() {
  ...
  switch (max) {
    ...
    case 3:
      ...
      if (autoBattle.bonuses.Extra_Limbs.level < 1) {
        items = ['Sword', 'Armor', 'Fists_of_Goo', 'Battery_Stick'];
        level = [4, 2, 2, 2];
        for (let equip in autoBattle.items) {
  ...
  let activeContract = autoBattle.items[autoBattle.activeContract]
  contractVoid = (activeContract != null && game.global.world >= activeContract.zone
