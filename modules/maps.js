    ...
    if (Object.keys(mapSpecialModifierConfig).forEach(function (o) {
        ...
        if (c) {
            if (59 <= game.global.highestLevelCleared) {
                ...
                for (let d = updateMapCost(!0), e = game.resources.fragments.owned, f = 100 * (d / e); 0 < c.selectedIndex && d > e;) {
                    c.selectedIndex -= 1;
                    "0" != c.value && console.log("Could not afford " + mapSpecialModifierConfig[c.value].name);
                }
                let d = updateMapCost(!0),
    ...
    if (runUniques || Rshoulddobogs || Rshouldtimefarmbogs) {
        for (var map in game.global.mapsOwnedArray) {
            ...
            if (Rshoulddobogs && theMap.name == 'The Black Bog') {
                ...
                var frozencastle = [10000, 10000];
                if (getPageSetting('Rfrozencastle')[0] > 0 && getPageSetting('Rfrozencastle')[1] >= 0) frozencastle = getPageSetting('Rfrozencastle');
                if (theMap.name == 'Frozen Castle' && (game.global.world >= frozencastle[0] && ((game.global.lastClearedCell + 1) >= frozencastle[1]))) {
